...
def predict(input_data, model=None, scaler_X=None, scaler_y=None, config=None):
    """
    ...
    Args:
        ...
        config: Configuración (si None, se carga automáticamente)

    Returns:
        dict con predicciones: {'P2.5': float, 'O3': float, 'CO': float}
    """
    # Cargar modelo si no se proporcionó
    if model is None or scaler_X is None or scaler_y is None or config is None:
