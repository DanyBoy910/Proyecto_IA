...
hojas_a_procesar = [
    ...
    {
        "sheet_name": "Óxido Nítrico (NO)",
        "var_name": "NO",
        "tipo": "secuencial_variable",
    },
    {
        "sheet_name": "Óxido de Nitrógeno (NxOy)",
        "var_name": "NxOy",
        "tipo": "secuencial_variable",
    },
    ...
    {
        "sheet_name": "Velocidad del Viento (VV)",
        "var_name": "VV",
        "tipo": "secuencial_variable",
    },
    {
        "sheet_name": "Humedad Relativa (HR)",
        "var_name": "HR",
        "tipo": "secuencial_variable",
    },
    {"sheet_name": "Temperatura (TEMP)", "var_name": "T", "tipo": "horario"},
    {
        "sheet_name": "Presión Barométrica (PB)",
        "var_name": "PB",
        "tipo": "secuencial_variable",
    },
    {
        "sheet_name": "Radiación Solar (RS)",
        "var_name": "RS",
        "tipo": "secuencial_variable",
    },
    {
        "sheet_name": "Precipitación Pluvial (PP)",
        "var_name": "PP",
        "tipo": "secuencial_variable",
    },
]
...
df_master = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [fechas_fijas, horas_fijas], names=["Fecha", "Hora"]
    )
).reset_index()
...
if not os.path.exists(archivo_excel):
    ...
    for hoja_info in hojas_a_procesar:
        ...
        try:
            ...
            if "Valor" in df.columns:
                columna_valor_nombre = "Valor"
            elif "valor" in df.columns:
                columna_valor_nombre = "valor"

            if not columna_valor_nombre:
                print(
                    f"  > ADVERTENCIA (Hoja {sheet_name}): No se encontró 'Valor' o 'valor'. Se omitirá."
                )
                continue
            ...
            if tipo_proceso == "horario":
                ...
                if "Hora" not in df.columns or df["Hora"].isnull().all():
                    ...
                    df_hourly.rename(
                        columns={columna_valor_nombre: var_name}, inplace=True
                    )
                    ...
                    df_hourly["Fecha"] = (
                        df_hourly["Fecha"].astype(str).str.split("T").str[0]
                    )
            ...
            # ---
                ...
                df_daily_seq = df[["Fecha", columna_valor_nombre]].rename(
                    columns={columna_valor_nombre: var_name}
                )
                df_daily_seq["Fecha"] = (
                    df_daily_seq["Fecha"].astype(str).str.split("T").str[0]
                )
                ...
                for fecha_dia, grupo_dia in df_daily_seq.groupby("Fecha"):
                    ...
                    if num_valores > 24:
                        print(
                            f"  > ADVERTENCIA (Hoja {sheet_name}): {fecha_dia} tiene {num_valores} filas. Se usarán solo las últimas 24."
                        )
                        valores_del_dia = valores_del_dia[-24:]
                        num_valores = 24

                    # --- La Lógica Clave ---
                    horas_target = horas_fijas[
                        -num_valores:
                    ]  # Obtenemos las últimas N horas

                    # Mapeamos los valores a las horas target
                    for i in range(num_valores):
                        hora_a_rellenar = horas_target[i]
                        valor_a_mapear = valores_del_dia[i]
                        df_master_idx.loc[(fecha_dia, hora_a_rellenar), var_name] = (
                            valor_a_mapear
                        )
    ...
    datos_json_final_ordenados = [
        mapa_fechas[fecha] for fecha in fechas_fijas if fecha in mapa_fechas
    ]
